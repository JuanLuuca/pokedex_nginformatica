...
export const FavoritePage = () => {
    ...
    const [getFavorites, setGetFavorites] = useState<typeGetFavorite[]>([])
    ...
    useEffect(() => {
        async function FavoriteGetApi() {
            const userid = localStorage.getItem('user')
    
            try {
                const response = await loginApi.get(`/getuser/${userid}`);
                const data = response.data
                setGetFavorites(data.user.favoritesPokemon)
            } catch (error) {
                return console.log('deu erro no endpoint')
            }
        }
            ...
            .map(data => (
                <div key={data._id} className="max-w-[1440px] m-auto">
                    <div className="h-44 w-[26rem] p-2 m-3 rounded-md bg-gradient-to-r from-zinc-200 to-zinc-100">
                        ...
                        <img src={capturadoImage.includes(data._id ) ? pokebolaimg : pokebolasemcor} alt="Capturar" title="Capturar" className="h-12 w-12 cursor-pointer" onClick={() => handleGetCapturePokemon(data._id, data.name, data.experiencia, data.peso, data.imagemPokemon)} />
                        <img src={favorito} alt="Favorito" title="Favoritar" className="h-14 w-16 ml-1" />
                        </div>
                    </div>
                </div>
            ))} 
